...
function wrapDataForPieChart(buckets, misCount) {
    ...
    for (var valueKey in buckets) {
        ...
        console.log(pieObject.label+ "=" +pieObject.value);
        label[i]=pieObject.label;
        quantities[i]=pieObject.value;
        i++;
...
//function to write to excel sheet
function export_excel(pieObjects){
var l="",q=""; var j=1;
    var file_name = prompt("Enter the file name:", "File name");
    var s=file_name;
    ...
    for(var i=0;i< len;i++){
        ...
        ex.write({"cell": str,"content": l});
        ex.write({"cell": str1,"content": q});
        j++;
       // ex.writeCell(str, label[i]);
      //  ex.writeCell(str1, quantities[i]);
    }
    label=[];
    quantities=[];
    ex.saveAs(s);
